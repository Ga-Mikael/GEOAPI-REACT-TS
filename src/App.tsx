...
import {IInfo, IInfoCountry, IName} from "./types/props";


function App() {
    const [countries, setCountries] = useState<IInfoCountry>()

    useEffect(() => {
        fetch("https://restcountries.com/v3.1/region/europe")
            .then(data => data.json())
            .then(data => {
                data.sort((a : IName, b: IName) => {
            ...
            <div className="max-w-7xl mx-auto py-20 px-4">
                ...
                {countries && (
                    <ul className="grid min-[450px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 auto-rows-[200px]">
                        {countries.map((country: IInfo, index: number) => (
